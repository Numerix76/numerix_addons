...
function ENT:SetMusicRadio(ply, url)
	...
	if idSC            then Radio.SetMusicSC (ply, self, idSC) end
    if !idYT and !idSC then Radio.SetMusicMP3(ply, self, url)  end
...
function ENT:ChangeModRadio(ply, radio)
    ...
    if self:GetControlerRadio() != self then
        radio:SetNWEntity("Radio:Entity", self:GetControlerRadio())
